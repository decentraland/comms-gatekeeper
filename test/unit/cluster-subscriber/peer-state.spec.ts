...
describe('cluster-subscriber peer state', () => {
  ...
  describe('when an assignment is stored', () => {
    it('should return it', () => {
      ...
      store.set('0xaaa', { clusterId: 'C1', shard: 0, room: 'island-C1', lastSeen: 1000 })

      expect(store.get('0xaaa')).toEqual({ clusterId: 'C1', shard: 0, room: 'island-C1', lastSeen: 1000 })
    })
  })
  ...
  describe('when the TTL elapses', () => {
    it('should expire the assignment, since the feed carries no disconnects', async () => {
      ...
      store.set('0xaaa', { clusterId: 'C1', shard: 0, room: 'island-C1', lastSeen: 0 })
  ...
  describe('when more wallets are stored than the bound allows', () => {
    it('should evict so memory stays bounded', () => {
      ...
      store.set('0xa', { clusterId: 'C1', shard: 0, room: 'island-C1', lastSeen: 1 })
      store.set('0xb', { clusterId: 'C1', shard: 0, room: 'island-C1', lastSeen: 2 })
      store.set('0xc', { clusterId: 'C1', shard: 0, room: 'island-C1', lastSeen: 3 })
