...
import {
  createBanEvent,
  createBanLiftedEvent,
  createWarningEvent,
  publishModerationEvent
} from '../../../src/logic/user-moderation/events'
...
describe('user-moderation events', () => {
  ...
  describe('when publishing a moderation event', () => {
    ...
    describe('and it succeeds', () => {
      ...
      beforeEach(() => {
        mockPublisher = createPublisherMockedComponent({ publishMessage: jest.fn().mockResolvedValueOnce(undefined) })
        mockLogger = createLoggerMockedComponent().getLogger('test') as jest.Mocked<ILoggerComponent.ILogger>
        ban = makeBan()
        event = createBanEvent(ban)
      })

      it('should publish the event via SNS', async () => {
        await publishModerationEvent(mockPublisher, event, mockLogger)
        expect(mockPublisher.publishMessage).toHaveBeenCalledWith(event)
      })
    })
    ...
    describe('and it fails after all retries', () => {
      ...
      it('should not throw', async () => {
        await expect(publishModerationEvent(mockPublisher, event, mockLogger)).resolves.toBeUndefined()
      })
    })
  })
})
