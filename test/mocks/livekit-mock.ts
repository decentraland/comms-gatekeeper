...
export const createLivekitMockedComponent = (
  ...
  return {
    ...
    getPrivateVoiceChatRoomName: jest.fn().mockImplementation((roomId: string) => `${PRIVATE_VOICE_CHAT_ROOM_PREFIX}${roomId}`),
    getCallIdFromRoomName: jest.fn().mockImplementation((roomName: string) => roomName.replace(PRIVATE_VOICE_CHAT_ROOM_PREFIX, '')),
    getCommunityVoiceChatRoomName: jest.fn().mockImplementation((communityId: string) => `${COMMUNITY_VOICE_CHAT_ROOM_PREFIX}-${communityId}`),
    getCommunityIdFromRoomName: jest.fn().mockImplementation((roomName: string) => roomName.replace(`${COMMUNITY_VOICE_CHAT_ROOM_PREFIX}-`, '')),
    getRoomName: jest.fn(),
