...
const BAN_SELECT_FIELDS = `id, banned_address as "bannedAddress", banned_by as "bannedBy", reason,
               custom_message as "customMessage", banned_device_id as "bannedDeviceId", banned_ip as "bannedIp",
...
export function createUserModerationDBComponent(components: {
  ...
  return {
    async createBan(input: CreateBanInput): Promise<UserBan> {
      ...
      const query = SQL`
        INSERT INTO user_bans (id, banned_address, banned_by, reason, custom_message, banned_device_id, banned_ip, banned_at, expires_at, created_at)
        VALUES (${id}, ${input.bannedAddress}, ${input.bannedBy}, ${input.reason}, ${input.customMessage ?? null}, ${input.bannedDeviceId ?? null}, ${input.bannedIp ?? null}, ${now}, ${input.expiresAt ?? null}, ${now})
    ...
    async getActiveBanForConnection({ address, deviceId, ip }: ConnectionBanQuery): Promise<BanStatus> {
      // Match an active ban by address OR by the captured device id / IP, so a banned player
      // is rejected even when reconnecting under a different wallet. Device/IP terms are only
      // added when provided (a null term must not match rows where that column is null).
      const identifierMatch = SQL`banned_address = ${address}`
      if (deviceId) {
        identifierMatch.append(SQL` OR banned_device_id = ${deviceId}`)
      }
      if (ip) {
        identifierMatch.append(SQL` OR banned_ip = ${ip}`)
      }
